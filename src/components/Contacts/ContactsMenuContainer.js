import React from 'react';
import ContactsMenu from "./ContactsMenu";
// import {Field, reduxForm} from "redux-form";
// import {Input, Textarea} from "../common/FormsControls/FormsControls";
import {postMailThunkCreator} from "../../redux/contacts-reducer";
import {useDispatch} from "react-redux";
import ContactsForm from "./ContactsFormContainer";




const ContactsMenuContainer = (props) => {

   return <>

      <div className={props.showContacts ? "contactsMenu _show" : "contactsMenu"}>
         <div className="backgroundPattern"></div>
         <svg className={"closeIcon"} onClick={()=>{props.setShowContacts(false)}} id="close" viewBox="0 0 12 12" fill="none"><path d="M10.1134 1.96826L1.56909 10.5125" /><path d="M1.56909 1.96826L10.1134 10.5125" /></svg>

         {/*<svg className={"closeIcon"} onClick={()=>{props.setShowContacts(false)}}><use href={svgSprite+"#close"}/></svg>*/}
         <ContactsMenu showContacts={props.showContacts} setShowContacts={props.setShowContacts}/>
         <ContactsForm/>

      </div>
   </>
}



export default ContactsMenuContainer



